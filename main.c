...
void buildFire() {
  ...
  if (choice == 1) {
    printf("\n\nYou have managed to get a fire started with the flint, sticks and dry leaves. You warm yourself up around the fire and pack the extra materials you gathered in your backpack so that you can make a fire again if you need to. Achievement unlocked: Fire building\n");
    fire = 1;
    stayPut();
  }
  else if (choice == 2) {
    printf("\n\n\tYou use the flint and are able to start a fire! You watch as the colours of the magazine cover begin to melt and the fire gives off a dark smoke. You lean in to warm yourself closer to the fire, and begin to feel dizzy. Little did you know, but the salt from the seawater in the driftwood and the paints and chemicals used in the ink of the magazine are giving off a toxic smoke as they burn. Your last thoughts are of regret that you didn't finish reading that book on how to survive in the woods that you got last Christmas. Everything fades to black... \n\t\t\tGAME OVER\n");
  finished = 1;
  }
}
...
void fixCellphone() {
  printf("\n\nYou open up your cellphone and see that it is completely out of battery. You vaguely remember reading an article on how to build your own battery powered charger. It seemed difficult, and you are without a soldering iron, but you think it might be worth a shot. You search around the area and miraculously find some copper wires on the ground. You dig out some batteries from the bottom of your backpack and try to remember the steps to build the charger. You manage to MacGyver a battery that you think will work, now you just have to decide how to connect the wires to your phone's battery. ");
  ...
  if (choice == 1) {
    cellphone = 1;
    printf("\n\nYou are able to get a charge on your cellphone! Achievement unlocked: Charged cellphone\nUnfortunately, there is no service where you are. You need to move to a new location in order to get cell service and call for help.\n");
    chooseNewLocation();
  }

  else if (choice == 2) {
    printf("\n\nYou eagerly push the power button on your phone and wait to see what happens. Nothing comes up on the screen but you feel the phone getting warm in your hands. You think that this must be a good sign, and wait as the phone gets warmer and warmer. Suddenly, you realize that you must have connected the wrong wires to the wrong terminals and that the battery must be short-circuiting! Unfortunately, you are not able to disconnect the wires in time and the phone blows up in your hands. A sharp piece of metal flies from the phone and into your temple, and you feel the blood starting to trickle down your face as everything fades to black...\n\t\t\tGAME OVER\n");
  }
}
...
void climbTree() {
  ...
  if (choice == 1)
    ...
    printf("\n\nYou open up the cellphone and to your surprise, there is cell service! You call 911 and get through to an operator.\nThey are able to find your location and they send a helicopter to save you.\n\t\t\tVICTORY!!\nCongratulations, you've won!\n");
    finished = 1;
}
